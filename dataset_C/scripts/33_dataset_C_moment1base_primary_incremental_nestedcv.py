...
# =============================================================================
# 33_dataset_C_moment1base_primary_incremental_nestedcv.py
#
# GENERIC FROZEN-REPRESENTATION COMPARATOR TEST
#
# Scientific question:
#   Does the prespecified frozen MOMENT-1-base mean768 representation add
...
#       M6a = C0a core context + frozen MOMENT-1-base mean768
#       M6b = C0b extended context + frozen MOMENT-1-base mean768
...
#   - no alternative MOMENT pooling search;
#   - no MOMENT residualization or adaptation;
#   - no MOMENT fine-tuning / adaptation;
...
OUTDIR = ROOT / "results" / "dataset_C_moment1base_imu_primary_nestedcv"
AUDITDIR = ROOT / "audit"
OUTDIR.mkdir(parents=True, exist_ok=True)
AUDITDIR.mkdir(parents=True, exist_ok=True)

FOLD_OUT = OUTDIR / "dataset_C_moment1base_primary_fold_metrics.csv"
PRED_OUT = OUTDIR / "dataset_C_moment1base_primary_predictions.csv"
AVG_OUT = OUTDIR / "dataset_C_moment1base_primary_avg_oof_predictions.csv"
SUMMARY_OUT = OUTDIR / "dataset_C_moment1base_primary_model_summary_with_locked_baselines.csv"
INCREMENT_OUT = OUTDIR / "dataset_C_moment1base_primary_incremental_value.csv"
ROBUST_OUT = OUTDIR / "dataset_C_moment1base_primary_alignment_robustness.csv"
HYPER_OUT = OUTDIR / "dataset_C_moment1base_primary_selected_hyperparameters.csv"
LOCKCHECK_OUT = OUTDIR / "dataset_C_moment1base_primary_baseline_lock_validation.csv"
AUDIT_OUT = AUDITDIR / "dataset_C_moment1base_imu_primary_nestedcv_audit_33.txt"

PARTIAL_FOLD = OUTDIR / "_partial_moment1base_primary_fold_metrics.csv"
PARTIAL_PRED = OUTDIR / "_partial_moment1base_primary_predictions.csv"
PARTIAL_HYPER = OUTDIR / "_partial_moment1base_primary_selected_hyperparameters.csv"
...
BASELINE_FEATURE_SETS = {
    "M6a_core_plus_moment1base_mean768": "C0a_core_context",
    "M6b_extended_plus_moment1base_mean768": "C0b_extended_context",
}

COMPARISON_LABELS = {
    "M6a_core_plus_moment1base_mean768": "Core-context -> MOMENT-1-base mean768 increment",
    "M6b_extended_plus_moment1base_mean768": "Extended-context -> MOMENT-1-base mean768 increment",
}
...
if start_sha != EXPECTED_START_EMB_SHA256:
    raise SystemExit(f"START MOMENT mean768 hash differs from Script-32 audit. Expected={EXPECTED_START_EMB_SHA256} Observed={start_sha}")
if end_sha != EXPECTED_END_EMB_SHA256:
    raise SystemExit(f"END MOMENT mean768 hash differs from Script-32 audit. Expected={EXPECTED_END_EMB_SHA256} Observed={end_sha}")
        ...
        for anchor, df in anchors.items():
            ...
            for model_name in ["ElasticNet", "HGB"]:
                for fs_idx, (fs_name, cols) in enumerate(FEATURE_SETS.items()):
                    ...
                    n_context = len(core_cols) if fs_name.startswith("M6a_") else len(extended_cols)
                    fold_rows.append({
                        "repeat": repeat,
    ...
    for model_idx, model_name in enumerate(["ElasticNet", "HGB"]):
        for comp_idx, combined_name in enumerate(FEATURE_SETS.keys()):
            ...
            inc_rows.append({
                "anchor": anchor,
                "model": model_name,
                "representation": "MOMENT-1-base_IMU_primary_mean768",
...
lines = []
add = lines.append
add("DATASET C — FROZEN MOMENT-1-BASE GENERIC-COMPARATOR INCREMENTAL NESTED-CV AUDIT")
add("=" * 118)
add(f"Rows per anchor: {len(start_df)}")
add(f"Participants: {start_df[GROUP].nunique()}")
add("Scientific question: does frozen MOMENT-1-base mean768 add value beyond the LOCKED 18f context baselines?")
...
add("Representation was extracted by frozen MOMENT-1-base before supervised downstream modeling.")
add("No representation tuning or pooling selection is performed here.")
add("")
add("2. LITERAL 18f BASELINE LOCK")
add("-" * 118)
add("C0a/C0b context baselines are NOT re-fit in Script 33.")
add("Their held-out predictions, fold metrics, averaged OOF predictions, and absolute metrics are loaded directly from 18f.")
add("Reconstructed Script-33 outer test row IDs were checked against every locked 18f baseline fold.")
...
add(f"M6a core + MOMENT-1-base mean768: {len(core_cols)+768} raw predictors.")
add(f"M6b extended + MOMENT-1-base mean768: {len(extended_cols)+768} raw predictors.")
add("")
...
add("PASS: frozen MOMENT-1-base embedding extraction used no target labels and no outer-test adaptation.")
...
add("")
add("6. MOMENT-1-BASE GENERIC COMPARATOR INCREMENTAL VALUE")
add("-" * 118)
show_cols = [
...
add("")
add("Sign convention:")
add("  delta_mae_pb  > 0 => MOMENT-1-base improves participant-balanced MAE.")
add("  delta_rmse_pb > 0 => MOMENT-1-base improves participant-balanced RMSE.")
add("  delta_r2_rep  > 0 => MOMENT-1-base improves pooled R2.")
add("")
...
add("-" * 118)
add("The primary estimand is M4-M0: context + frozen MOMENT-1-base representation versus the corresponding locked context baseline.")
...
add("-" * 118)
add(f"DATASET C MOMENT-1-BASE GENERIC COMPARATOR INCREMENTAL BENCHMARK PASS: {global_pass}")
if global_pass:
...
add("=" * 118)
add("END OF MOMENT-1-BASE GENERIC COMPARATOR INCREMENTAL BENCHMARK")
...
print(" ", ROBUST_OUT)
print("")
print(f"DATASET C MOMENT-1-BASE GENERIC COMPARATOR INCREMENTAL BENCHMARK PASS: {global_pass}")
print("Upload these three files to ChatGPT:")
print("  dataset_C_moment1base_imu_primary_nestedcv_audit_33.txt")
print("  dataset_C_moment1base_primary_incremental_value.csv")
print("  dataset_C_moment1base_primary_alignment_robustness.csv")
