...
OUTDIR = SRC27 / "convergence_audit_33c"
OUTDIR.mkdir(parents=True, exist_ok=True)
AUDITDIR = ROOT / "audit"
AUDITDIR.mkdir(parents=True, exist_ok=True)

FIT_OUT = OUTDIR / "dataset_C_moment1base_problem_candidate_recovery_fit_audit.csv"
TASK_OUT = OUTDIR / "dataset_C_moment1base_problem_candidate_recovery_task_summary.csv"
AUDIT_OUT = AUDITDIR / "dataset_C_moment1base_cls_problem_candidate_recovery_audit_33c.txt"
...
for ids, emb, tag in [
    ...
    if not np.array_equal(ids, locked_ids):
        raise SystemExit(f"{tag} MOMENT embedding row IDs mismatch.")
    if emb.shape != (1141, 768):
        raise SystemExit(f"{tag} unexpected embedding shape {emb.shape}.")
    if not np.isfinite(emb).all():
        raise SystemExit(f"{tag} non-finite MOMENT embedding.")
...
lines = []
add = lines.append
add("DATASET C — SCRIPT 33 MOMENT-1-BASE PROBLEM-CANDIDATE RECOVERY AUDIT (33c)")
add("=" * 118)
add("Purpose: recover only the one non-converged Script-33 ElasticNet grid candidate (alpha=0.01, l1_ratio=0.1).")
...
add(f"SCRIPT-33 ELASTICNET SELECTION STABLE AFTER RECOVERY: {selection_stable}")
add("")
add("4. DECISION")
add("-" * 118)
if selection_stable:
    add("The only non-converged Script-33 grid candidate was recovered numerically and did not beat the originally selected candidate in any task.")
    add("Therefore the prior ConvergenceWarnings do not alter Script-33 ElasticNet hyperparameter selection.")
    add("No Script-33 rerun is required for this convergence issue.")
else:
    add("Do NOT lock the Script-33 ElasticNet scientific result yet.")
...
print(f"SCRIPT-33 ELASTICNET SELECTION STABLE AFTER RECOVERY: {selection_stable}")
