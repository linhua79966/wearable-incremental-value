...
# =============================================================================
# 27_dataset_C_normwear_imu_cls_sensitivity_nestedcv.py
#
# PRESPECIFIED CLS-POOLING SENSITIVITY TEST
#
# Scientific question:
#   Does the prespecified frozen NormWear-IMU CLS/first-patch sensitivity
#   representation add stable predictive value beyond the LOCKED 18f context baselines?
#
# IMPORTANT DESIGN:
...
#       S1a = C0a core context + frozen NormWear CLS768
#       S1b = C0b extended context + frozen NormWear CLS768
...
#   - CLS768 is evaluated here only as the prespecified sensitivity representation;
#   - no residualized Foundation representation (M5);
#   - no NormWear fine-tuning / adaptation;
#   - no outcome-driven representation selection;
#   - CLS768 cannot replace the locked mean768 primary analysis regardless of result.
# =============================================================================
...
OUTDIR = ROOT / "results" / "dataset_C_normwear_imu_cls_sensitivity_nestedcv"
AUDITDIR = ROOT / "audit"
OUTDIR.mkdir(parents=True, exist_ok=True)
AUDITDIR.mkdir(parents=True, exist_ok=True)

FOLD_OUT = OUTDIR / "dataset_C_normwear_cls_sensitivity_fold_metrics.csv"
PRED_OUT = OUTDIR / "dataset_C_normwear_cls_sensitivity_predictions.csv"
AVG_OUT = OUTDIR / "dataset_C_normwear_cls_sensitivity_avg_oof_predictions.csv"
SUMMARY_OUT = OUTDIR / "dataset_C_normwear_cls_sensitivity_model_summary_with_locked_baselines.csv"
INCREMENT_OUT = OUTDIR / "dataset_C_normwear_cls_sensitivity_incremental_value.csv"
ROBUST_OUT = OUTDIR / "dataset_C_normwear_cls_sensitivity_alignment_robustness.csv"
HYPER_OUT = OUTDIR / "dataset_C_normwear_cls_sensitivity_selected_hyperparameters.csv"
LOCKCHECK_OUT = OUTDIR / "dataset_C_normwear_cls_sensitivity_baseline_lock_validation.csv"
AUDIT_OUT = AUDITDIR / "dataset_C_normwear_imu_cls_sensitivity_nestedcv_audit.txt"

PARTIAL_FOLD = OUTDIR / "_partial_normwear_cls_sensitivity_fold_metrics.csv"
PARTIAL_PRED = OUTDIR / "_partial_normwear_cls_sensitivity_predictions.csv"
PARTIAL_HYPER = OUTDIR / "_partial_normwear_cls_sensitivity_selected_hyperparameters.csv"
...
BASELINE_FEATURE_SETS = {
    "S1a_core_plus_normwear_cls768": "C0a_core_context",
    "S1b_extended_plus_normwear_cls768": "C0b_extended_context",
}

COMPARISON_LABELS = {
    "S1a_core_plus_normwear_cls768": "Core-context -> NormWear CLS768 sensitivity increment",
    "S1b_extended_plus_normwear_cls768": "Extended-context -> NormWear CLS768 sensitivity increment",
}
...
if start_sha != EXPECTED_START_EMB_SHA256:
    raise SystemExit(f"START CLS768 hash differs from Script-24 audit. Expected={EXPECTED_START_EMB_SHA256} Observed={start_sha}")
if end_sha != EXPECTED_END_EMB_SHA256:
    raise SystemExit(f"END CLS768 hash differs from Script-24 audit. Expected={EXPECTED_END_EMB_SHA256} Observed={end_sha}")
        ...
        for anchor, df in anchors.items():
            ...
            for model_name in ["ElasticNet", "HGB"]:
                for fs_idx, (fs_name, cols) in enumerate(FEATURE_SETS.items()):
                    ...
                    n_context = len(core_cols) if BASELINE_FEATURE_SETS[fs_name] == "C0a_core_context" else len(extended_cols)
                    fold_rows.append({
                        "repeat": repeat,
...
for (anchor, model_name, fs_name), g in avg.groupby(["anchor", "model", "feature_set"]):
    ...
    new_summary_rows.append({
        "anchor": anchor,
        "source": "27_new_normwear_cls_sensitivity",
        "model": model_name,
        "feature_set": fs_name,
        "n_rows": len(g),
    ...
    for model_idx, model_name in enumerate(["ElasticNet", "HGB"]):
        for comp_idx, combined_name in enumerate(FEATURE_SETS.keys()):
            ...
            inc_rows.append({
                "anchor": anchor,
                "model": model_name,
                "representation": "NormWear_IMU_CLS_sensitivity_cls768",
...
lines = []
add = lines.append
add("DATASET C — PRESPECIFIED NORMWEAR-IMU CLS SENSITIVITY NESTED-CV AUDIT")
add("=" * 118)
add(f"Rows per anchor: {len(start_df)}")
add(f"Participants: {start_df[GROUP].nunique()}")
add("Scientific question: does the prespecified frozen NormWear-IMU CLS768 sensitivity representation add value beyond the LOCKED 18f context baselines?")
...
add("CLS768 was prespecified in Script 24 as sensitivity-only; no result-driven pooling selection is performed here.")
add("")
add("2. LITERAL 18f BASELINE LOCK")
add("-" * 118)
add("C0a/C0b context baselines are NOT re-fit in Script 27.")
add("Their held-out predictions, fold metrics, averaged OOF predictions, and absolute metrics are loaded directly from 18f.")
add("Reconstructed Script-27 outer test row IDs were checked against every locked 18f baseline fold.")
...
add(f"S1a core + NormWear CLS768: {len(core_cols)+768} raw predictors.")
add(f"S1b extended + NormWear CLS768: {len(extended_cols)+768} raw predictors.")
add("")
...
add("PASS: frozen NormWear CLS768 extraction used no target labels and no outer-test adaptation.")
...
add("")
add("6. PRESPECIFIED CLS SENSITIVITY INCREMENTAL VALUE")
add("-" * 118)
show_cols = [
...
add("")
add("Sign convention:")
add("  delta_mae_pb  > 0 => NormWear CLS sensitivity improves participant-balanced MAE.")
add("  delta_rmse_pb > 0 => NormWear CLS sensitivity improves participant-balanced RMSE.")
add("  delta_r2_rep  > 0 => NormWear CLS sensitivity improves pooled R2.")
add("")
...
add("-" * 118)
add("This is a prespecified pooling sensitivity analysis: context + frozen NormWear CLS768 versus the corresponding locked 18f context baseline.")
add("No foundation-representation-only model is used to infer incremental value in this script.")
add("CLS768 is evaluated only as the prespecified sensitivity representation and cannot replace the locked mean768 primary analysis.")
...
add("-" * 118)
add(f"DATASET C NORMWEAR CLS SENSITIVITY BENCHMARK PASS: {global_pass}")
if global_pass:
...
add("=" * 118)
add("END OF PRESPECIFIED NORMWEAR CLS SENSITIVITY BENCHMARK")
...
print(" ", ROBUST_OUT)
print("")
print(f"DATASET C NORMWEAR CLS SENSITIVITY BENCHMARK PASS: {global_pass}")
print("Upload these three files to ChatGPT:")
print("  dataset_C_normwear_imu_cls_sensitivity_nestedcv_audit.txt")
print("  dataset_C_normwear_cls_sensitivity_incremental_value.csv")
print("  dataset_C_normwear_cls_sensitivity_alignment_robustness.csv")
