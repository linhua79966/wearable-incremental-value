...
# =============================================================================
# 27b_dataset_C_normwear_cls_elasticnet_convergence_audit.py
#
# READ-ONLY TECHNICAL AUDIT FOR SCRIPT 27
#
# Purpose:
#   Reproduce the ElasticNet path of Script 27 without changing any model,
#   hyperparameter, split, representation, baseline, or scientific result.
#
# It checks:
#   1) all 12 ElasticNet grid candidates x 4 inner folds for every Script-27
...
#   6) reproduction of the stored Script-27 outer-test predictions.
#
# IMPORTANT:
...
PRED_IN = SRC27 / "dataset_C_normwear_cls_sensitivity_predictions.csv"

OUTDIR = SRC27 / "convergence_audit_27b"
OUTDIR.mkdir(parents=True, exist_ok=True)
AUDITDIR = ROOT / "audit"
AUDITDIR.mkdir(parents=True, exist_ok=True)

FIT_OUT = OUTDIR / "dataset_C_normwear_cls_elasticnet_convergence_fit_audit.csv"
CAND_OUT = OUTDIR / "dataset_C_normwear_cls_elasticnet_convergence_candidate_summary.csv"
TASK_OUT = OUTDIR / "dataset_C_normwear_cls_elasticnet_convergence_task_summary.csv"
AUDIT_OUT = AUDITDIR / "dataset_C_normwear_cls_elasticnet_convergence_audit_27b.txt"
...
EXPECTED_PRED_SHA256 = "4FD4980C310230A47C40B583358E1EE9E0BB888A4B8CEB3B2E028C693347F7D7"
...
FEATURE_SET_NAMES = [
    "S1a_core_plus_normwear_cls768",
    "S1b_extended_plus_normwear_cls768",
]
...
if hyper_sha != EXPECTED_HYPER_SHA256:
    raise SystemExit(f"Script-27 selected-hyperparameter hash mismatch: {hyper_sha}")
if pred_sha != EXPECTED_PRED_SHA256:
    raise SystemExit(f"Script-27 prediction hash mismatch: {pred_sha}")
...
if not np.array_equal(s_ids, locked_ids) or not np.array_equal(e_ids, locked_ids):
    raise SystemExit("CLS embedding row IDs do not match locked rows.")
if s_emb.shape != (1141, 768) or e_emb.shape != (1141, 768):
    raise SystemExit("Unexpected CLS embedding shape.")
if not np.isfinite(s_emb).all() or not np.isfinite(e_emb).all():
    raise SystemExit("Non-finite CLS embedding.")
...
if len(hyper) != 100:
    raise SystemExit(f"Expected 100 Script-27 ElasticNet selected rows, found {len(hyper)}.")
    ...
    for outer_fold in range(1, OUTER_FOLDS + 1):
        ...
        for anchor, df in anchors.items():
            ...
            for fs_idx, (fs_name, cols) in enumerate(FEATURE_SETS.items()):
                ...
                # Reproduce Script-27 tie-breaking exactly.
...
lines = []
add = lines.append
add("DATASET C — SCRIPT 27 ELASTICNET CONVERGENCE AUDIT (27b)")
add("=" * 118)
...
add(f"Script-27 prediction SHA256: {pred_sha}")
add("")
add("2. REPRODUCTION")
add("-" * 118)
...
if reproduction_pass and selected_path_pass and full_grid_pass:
    add("All Script-27 ElasticNet fits reproduced and converged under the original max_iter=30000.")
    ...
    add("One or more originally selected ElasticNet paths did not converge and/or the Script-27 run did not reproduce exactly.")
    add("Do NOT interpret the CLS sensitivity result as locked until this technical issue is resolved.")
